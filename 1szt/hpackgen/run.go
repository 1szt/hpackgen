...
func Run() {
	...
	env.Init([][]string{
		...
		{"MANIFEST_VERSION", "1szt.{rand:9}",
			"版本号（整合包版本）",
			"",
			"支持模板占位符，运行时自动替换：",
			"  {date}      → 当前日期，如 20260714",
			"  {time}      → 当前时间，如 143052",
			"  {datetime}  → 当前日期+时间，如 20260714143052",
			"  {rand:N}    → N 位随机数字，如 {rand:4} → 8371",
			"  {shortuuid} → 8 位随机十六进制，如 a3f1c9e2",
			"",
			"默认规则：1szt.9位随机数",
			"也可使用时间戳等，例如：",
			"  {datetime}           → 20260714143052（完整时间戳）",
			"  {date}               → 20260714（仅日期）",
			"  v{date}-{shortuuid}  → v20260714-a3f1c9e2（日期+唯一标识）",
			"  1.0.0                → 固定版本（不含 {} 则不替换）"},
